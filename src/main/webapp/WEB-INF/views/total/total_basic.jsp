...
<body>
	...
	<hr id="redHr">
	<form method="post" action="<%=request.getContextPath()%>/total_home.do">
		<div id="userInput" align="center">

			
			<!-- <h2>성별은 무엇인가요?</h2> -->
			<input type="hidden" name="code" value="<%=code%>"/>
			<input type="hidden" id="input1" name="gender"  value="<%=session.getAttribute("gender")%>"/>
<%-- 			<input type="radio" id="gender_m" name="gender" value="1" checked>
				...
				<hr> --%>
			<!-- <h2>나이가 어떻게 되십니까?</h2> -->
			<input type="hidden" id="input1" name="nowAge"  value="<%=session.getAttribute("nowAge")%>"/>
<hr>
			<h4>총 수입 : <%=income %>만원</h4>
			<h4>총 지출 : <%=pay %>만원</h4>
